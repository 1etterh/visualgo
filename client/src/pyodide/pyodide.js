...
var loadPyodide = (()=>{
    ...
    var f = (t,e)=>_(t, "name", {
      ...
      , g = (t=>typeof require < "u" ? require : typeof Proxy < "u" ? new Proxy(t,{
    ...
    ;
    var h = (t,e,c)=>(c = t != null ? ce(fe(t)) : {},
    M(e || !t || !t.__esModule ? _(c, "default", {
        value: t,
        enumerable: !0
    }) : c, t))
      , me = t=>M(_({}, "__esModule", {
    ...
    var j = $((P,C)=>{
        (function(t, e) {
            "use strict";
            typeof define == "function" && define.amd ? define("stackframe", [], e) : typeof P == "object" ? C.exports = e() : t.StackFrame = e()
        }
        )(P, function() {
            "use strict";
            function t(d) {
            ...
            s.prototype = {
                ...
                toString: function() {
                    ...
                    return this.getIsEval() ? d ? "[eval] (" + d + ":" + y + ":" + w + ")" : "[eval]:" + y + ":" + w : E ? E + " (" + d + ":" + y + ":" + w + ")" : d + ":" + y + ":" + w
                }
            },
    ...
    var W = $((x,B)=>{
        (function(t, e) {
            "use strict";
            typeof define == "function" && define.amd ? define("error-stack-parser", ["stackframe"], e) : typeof x == "object" ? B.exports = e(j()) : t.ErrorStackParser = e(t.StackFrame)
        }
        )(x, f(function(e) {
            "use strict";
            var c = /(^|@)\S+:\d+/
...
`).filter(function(i) {
                    ...
                    return u.map(function(i) {
                        ...
                        var s = i.replace(/^\s+/, "").replace(/\(eval code/g, "(").replace(/^.*?\s+/, "")
                          , l = s.match(/ (\(.+\)$)/);
                        s = l ? s.replace(l[0], "") : s;
...
`).filter(function(i) {
                    ...
                    return u.map(function(i) {
                        var s = i.split("@"), l = this.extractLocation(s.pop()), m = s.shift() || "", p = m.replace(/<anonymous function(: (\w+))?>/, "$2").replace(/\([^)]*\)/g, "") || void 0, d;
    ...
    var G = h(W());
    var v = typeof process == "object" && typeof process.versions == "object" && typeof process.versions.node == "string" && typeof process.browser > "u"
      , F = v && typeof module < "u" && typeof module.exports < "u" && typeof g < "u" && typeof __dirname < "u"
      , H = v && !F
      , ye = typeof Deno < "u"
      , z = !v && !ye
      , q = z && typeof window < "u" && typeof document < "u" && typeof document.createElement < "u" && typeof sessionStorage < "u"
      , V = z && typeof importScripts < "u" && typeof self < "u";
    var K, k, L, X, D, ge = `"fetch" is not defined, maybe you're using node < 18? From Pyodide >= 0.25.0, node >= 18 is required. Older versions of Node.js may work, but it is not guaranteed or supported. Falling back to "node-fetch".`;
    async function A() {
        if (!v || (K = (await import(/* webpackIgnore */
        "url")).default,
        D = await import(/* webpackIgnore */
        "fs/promises"),
        globalThis.fetch ? k = fetch : (console.warn(ge),
        k = (await import(/* webpackIgnore */
        "node-fetch")).default),
        X = (await import(/* webpackIgnore */
        "vm")).default,
        L = await import(/* webpackIgnore */
        "path"),
        I = L.sep,
        typeof g < "u"))
            return;
        let t = await import(/* webpackIgnore */
        "fs")
          , e = await import(/* webpackIgnore */
        "crypto")
          , c = await import(/* webpackIgnore */
        "ws")
          , o = await import(/* webpackIgnore */
        "child_process")
          , a = {
            fs: t,
            crypto: e,
    ...
    v || (I = "/");
    function we(t, e) {
        return t.startsWith("file://") && (t = t.slice(7)),
        t.includes("://") ? {
            response: k(t)
        } : {
            binary: D.readFile(t).then(c=>new Uint8Array(c.buffer,c.byteOffset,c.byteLength))
        }
    }
    ...
    var S;
    if (q)
        S = f(async t=>await import(/* webpackIgnore */
        t), "loadScript");
    else if (V)
        S = f(async t=>{
            try {
    ...
    async function Ee(t) {
        ...
        t.includes("://") ? X.runInThisContext(await (await k(t)).text()) : await import(/* webpackIgnore */
        K.pathToFileURL(t).href)
    }
    f(Ee, "nodeLoadScript");
    async function Y(t) {
        if (v) {
    ...
    async function Q() {
        ...
        if (H) {
            let o = await import(/* webpackIgnore */
            "path");
            return (await import(/* webpackIgnore */
            "url")).fileURLToPath(o.dirname(e))
        }
        ...
        let e = t.FS
          ...
          , a = {
            ...
            getRemoteSet: async r=>{
                ...
                for (let[i,s] of u)
                    i !== "." && (n[o.join2(r.mountpoint, i)] = {
                        timestamp: s.kind === "file" ? (await s.getFile()).lastModifiedDate : new Date,
            ...
            loadLocalEntry: r=>{
                ...
                if (e.isFile(i.mode))
                    return u.contents = c.getFileDataAsTypedArray(u),
                    {
                        timestamp: i.mtime,
                        mode: i.mode,
                        contents: u.contents
            ...
            storeLocalEntry: (r,n)=>{
                if (e.isDir(n.mode))
                    ...
                    e.writeFile(r, n.contents, {
                        canOwn: !0
                    });
                else
                    throw new Error("node type not supported");
                e.chmod(r, n.mode),
                e.utime(r, n.timestamp, n.timestamp)
            }
            ,
            removeLocalEntry: r=>{
                var n = e.stat(r);
            ...
            storeRemoteEntry: async(r,n,u)=>{
                let i = r.get(o.dirname(n))
                  , s = e.isFile(u.mode) ? await i.getFileHandle(o.basename(n), {
                    create: !0
                }) : await i.getDirectoryHandle(o.basename(n), {
                    create: !0
                });
                if (s.kind === "file") {
                    let l = await s.createWritable();
                    await l.write(u.contents),
                    await l.close()
                }
                r.set(n, s)
            }
            ...
            removeRemoteEntry: async(r,n)=>{
                await r.get(o.dirname(n)).removeEntry(o.basename(n)),
                r.delete(n)
            }
            ,
            reconcile: async(r,n,u)=>{
                ...
                Object.keys(n.entries).forEach(function(p) {
                    let d = n.entries[p]
                      , y = u.entries[p];
                    (!y || e.isFile(d.mode) && d.timestamp.getTime() > y.timestamp.getTime()) && (s.push(p),
                    i++)
                }),
                s.sort();
                let l = [];
                if (Object.keys(u.entries).forEach(function(p) {
                    n.entries[p] || (l.push(p),
                    i++)
                }),
                l.sort().reverse(),
                !i)
    ...
    function ke(t, e) {
        t.preRun.push(()=>{
            for (let c of e)
                t.FS.mkdirTree(c),
                t.FS.mount(t.FS.filesystems.NODEFS, {
                    root: c
                }, c)
        }
        )
    ...
    function re(t, e) {
        ...
        t.instantiateWasm = function(a, r) {
            return async function() {
                try {
                    let n;
                    o ? n = await WebAssembly.instantiateStreaming(o, a) : n = await WebAssembly.instantiate(await c, a);
                    let {instance: u, module: i} = n;
                    typeof WasmOffsetConverter < "u" && (wasmOffsetConverter = new WasmOffsetConverter(wasmBinary,i)),
                    r(u, i)
                } catch (n) {
    ...
    async function T(t={}) {
        ...
        a.API = r,
        r.lockFilePromise = Y(o.lockFileURL),
        re(a, e),
        te(a, o);
        let n = new Promise(s=>a.postRun = s);
        if (a.locateFile = s=>o.indexURL + s,
        typeof _createPyodideModule != "function") {
            let s = `${o.indexURL}pyodide.asm.js`;
            await S(s)
        }
        if (await _createPyodideModule(a),
        await n,
        a.exited)
            throw a.exited.toThrow;
        if (t.pyproxyToStringRepr && r.setPyProxyToStringMethod(!0),
        r.version !== b)
            throw new Error(`Pyodide version does not match: '${b}' <==> '${r.version}'. If you updated the Pyodide version, make sure you also updated the 'indexURL' parameter passed to loadPyodide.`);
        a.locateFile = s=>{
            throw new Error("Didn't expect to load any more file_packager files!")
        }
        ;
        let u = r.finalizeBootstrap();
        if (u.version.includes("dev") || r.setCdnUrl(`https://cdn.jsdelivr.net/pyodide/v${u.version}/full/`),
        await r.packageIndexReady,
        r._pyodide._importhook.register_module_not_found_hook(r._import_name_to_package_name, r.lockfile_unvendored_stdlibs_and_test),
        r.lockfile_info.version !== b)
            throw new Error("Lock file version doesn't match Pyodide version");
        return r.package_loader.init_loaded_packages(),
